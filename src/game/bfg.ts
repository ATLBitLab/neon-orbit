 ...
 * you get, and it is paid in the same currency the enemy pays. A held Shield
 * does not change this: the Shield is for the gunfight, not for standing in a
 * fusion blast. A blast that is safe to stand inside is a free button.
 *
 ...
 * own wing, and neither is a fight anyone wants. Each human seat carries its
 * own two charges.
 *
...
import type { Faction } from './bolts'
...
export interface BfgTarget {
  ...
  takeDamage(amount: number, from: Faction, pierceShield?: boolean): void
}
...
export interface Bfg {
  ...
  /**
   * True when a hold this frame is currently costing the guns — already
   * spooling, or about to start. Gated on the *current* trigger, not last
   * frame's spooling flag: `Ship.step` runs before `update`, so a first-frame
   * check against `spooling` alone lets a ready gun fire.
   */
  wouldCharge(hold: boolean, owner: BfgTarget | null): boolean
...
export function createBfg(accent = 0x9dff3b): Bfg {
  ...
  let recovery = 0
  /**
   * A launch or abort consumes the trigger. Holding through the recovery
   * window must not spool the next round on its own — two scarce shots from
   * one long hold is the opposite of the design.
   */
  ...
  function canBegin(hold: boolean, owner: BfgTarget | null): boolean {
    return (
      hold &&
      !needsRelease &&
      recovery <= 0 &&
  ...
  function detonate(round: Round, frame: BfgFrame): BfgEvent {
    ...
    for (const target of frame.targets) {
      ...
      const own = target.faction === round.faction
      ...
      // A fusion blast is not a bolt. Shield is for the gunfight; standing in
      // this sphere is supposed to hurt, including the pilot who fired it.
      target.takeDamage(damage, round.faction, true)
  ...
  return {
    ...
    wouldCharge(hold, owner) {
      return spoolTimer > 0 || canBegin(hold, owner)
    },
    ...
    update(dt, frame) {
      ...
      if (!frame.hold) needsRelease = false

      const canSpool = canBegin(frame.hold, frame.owner)
